...
function deepDup(arr) {
  ...
  if (!Array.isArray(arr)) {
    // if not an array, return the input itself (base case for recursion)
    return arr;
  }

  // map over each element in the array
  return arr.map(element => {
    // recursively call deepDup on each element
    // this ensures that nested arrays are also deeply duplicated
